...
/**
 * Graph represents a mutable set of vertices connected by directed, labelled edges,
 * where singular edges are not bidirectional. Vertices may have edges connected to themselves.
 * Examples of Graphs include the "Empty Graph", "Node1", "Node1(5) Node2",
 *      "Node1(6) Node2
 *       Node2(3) Node1" and
 *
 *      "Node1(1) Node2
...
public class Graph<N, E> {
    ...
    // forall N n in m.keySet(), n != null &&
    // forall Sets s in m.get(n), s != null &&
    // forall E e in q, e != null

    /**
     * Holds all the nodes as keys in this graph, as well as all children of parent nodes,
     * and all unique edges.
     */
    ...
    public boolean addEdge(N parent, N child, E label) {
        ...
        if (!this.contains(parent) || !this.contains(child)) {
            throw new IllegalArgumentException("Graph does not contain one of the specified parent/child nodes.");
        }
        boolean res = true;
    ...
    public Set<N> getChildren(N parent) {
        if (!this.contains(parent)) {
            throw new IllegalArgumentException("Graph does not contain node: " + parent);
        }
