...
/**
 * This represents an immutable path between two objects, that can be thought of as nodes,
 * in a graph or any other network of node-like objects, particularly Path#getStart() and
 * Path#getEnd(). Also contains a cached version of the total cost along this path, for
...
public class Path<N> implements Iterable<Path<N>.Segment> {
    ...
    /**
     * Creates a new, empty path containing a starting object. Essentially this represents a path
     * from the start object to itself with a total cost of "0".
     *
     * @param start The starting object in the path.
     */
    public Path(N start) {
        this.start = start;
        this.cost = 0;
    ...
    /**
     * @return The object at the end of this path, which may be the starting object if this path
     * contains no segments (i.e. this path is from the starting object to itself).
     */
    public N getEnd() {
        if(path.size() == 0) {
            return start;
     ...
     * the starting object and itself, this iterator contains no elements. This iterator does not
